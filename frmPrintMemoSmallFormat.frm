...
Begin VB.Form frmPrintMemoSmallFormat 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.ComboBox cmbPrintFormat 
         ...
         ItemData        =   "frmPrintMemoSmallFormat.frx":0000
         Left            =   0
         List            =   "frmPrintMemoSmallFormat.frx":0002
         TabIndex        =   9
         Text            =   "cmbMemoFormat"
         Top             =   0
         Width           =   1590
      ...
      Begin VB.ComboBox cmbPaperSizes 
         ...
         Height          =   315
         IntegralHeight  =   0   'False
         ItemData        =   "frmPrintMemoSmallFormat.frx":0004
         Left            =   0
         List            =   "frmPrintMemoSmallFormat.frx":0006
         TabIndex        =   4
         Text            =   "cmbPaperSizes"
         Top             =   330
         Width           =   1590
   ...
   Begin VSPrinter8LibCtl.VSPrinter vp 
      ...
      PalettePicture  =   "frmPrintMemoSmallFormat.frx":0008
      AbortWindow     =   -1  'True
      AbortWindowPos  =   0
   ...
   End
End
Attribute VB_Name = "frmPrintMemoSmallFormat"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'**********************************************************************
'  PRINT BILL *HALF SIZE PAGE* WITH FIXED HEADER/FOOTER POSITIONS
'**********************************************************************
...
Public Sub RenderMemo(ByVal printFormatOption As Integer)
    ...
    If CN(0).recs.RecordCount = 1 Then
        CustNo = CN(0).recs!id: pdfFileName = Support & "_" & Format(Ref, "0000") & ".pdf"
     ...
     '   CN(1).dbOpen "SELECT Serial as Sl, itemID, ISBN as CODE, itemNAME AS Title, Authors, PublisherID AS PubID, PublisherName AS Pub, Pkg, Qty, CURRENCY as Cur, INRPrice as Price, Gross, Disc, Amount FROM " & Support & " WHERE DBRefX=" & Ref & "  ORDER BY SERIAL", 1
        CN(1).dbOpen "SELECT Serial as Sl, itemID, ISBN as CODE, itemNAME AS itemName, Authors, PublisherID AS PubID, PublisherName AS Pub, Pkg, Qty, CURRENCY as Cur, INRPrice as Price, Gross, Disc as 'Disc %', Amount FROM " & Support & " WHERE DBRefX=" & Ref & "  ORDER BY SERIAL", 1
        ...
        End Select
        vp.StartDoc
            SetFlxFont "[Render_Section_F]": Render_Section_F printFormatOption, 0
        vp.EndDoc
        
        i = vp.PageCount
        For i = 1 To vp.PageCount
            vp.StartOverlay i
                vp.TextAlign = taRightTop: vp.CurrentY = 250: vp.Text = "{ " & "Page " & i & " of " & vp.PageCount & " \par\b " & " }"
                SetFont "[Render_Section_A]": Render_Section_A printFormatOption, i
                SetFont "[Render_Section_B]": Render_Section_B printFormatOption, i
                SetFont "[Render_Section_C]": Render_Section_C printFormatOption, i
                SetFont "[Render_Section_D]": Render_Section_D printFormatOption, i
                'SetFont "[Render_Section_E]": Render_Section_E printFormatOption , i     'Conversion rates
                
                
                SetFont "[Render_Section_G]": Render_Section_G printFormatOption, i
                SetFont "[Render_Section_H]": Render_Section_H printFormatOption, i
                SetFont "[Render_Section_I]": Render_Section_I printFormatOption, i
                SetFont "[Render_Section_J]": Render_Section_J printFormatOption, i
                SetFont "[Render_Section_K]": Render_Section_K printFormatOption, i
            vp.EndOverlay
        Next
        '**********************************************************************
        '**********************************************************************
        ...
        Case Else:
            With vp
                .StartTable
                    ...
                    .TableCell(tcColAlign, , 2) = taCenterLeft
                  '  .TableCell(tcText, 1, 2) = "{" & AboutCompany & vbCrLf & "\par \b (" & CompanyDivision & " DIVISION)    CIN No.: U80302JH2007PTC012887, PAN: AADCB4036F" & "}"
                .EndTable
                .StartTable
                    .TableBorder = tbNone
                    .TableCell(tcCols) = 2: .TableCell(tcRows) = 1
    ...
    Select Case printFormatOption
        Case 1:
                ...
                .StartTable
                    ...
                    .TableCell(tcText, 1, 1) = "{" & Salutation & "\b " & CN(0).recs!Name & "\b0(" & ")" & "\par " & CN(0).recs!Address & "\par " & CN(0).recs!City & ". Ph: " & CN(0).recs!Phones & "}"
...
Public Sub Render_Section_F(ByVal printFormatOption As Integer, ByVal PageNum As Integer)
    ...
    Select Case printFormatOption
        Case 0: 'GENERAL
            ...
            flxItems.ColWidth(13) = "1315"    'AMOUNT
        Case 1: 'CHALLAN
            flxItems.ColWidth(0) = "500"    'SERIAL
            flxItems.ColWidth(1) = "0"      'itemID
    ...
    For i = 1 To flxItems.ROWS - 2
        If Trim(UCase(flxItems.TextMatrix(i, PublisherIDCol))) = "P592" And chkPNotes.Value = 1 Then
            flxItems.TextMatrix(i, ISBNCol) = "ISBN"
            flxItems.TextMatrix(i, itemNameCol) = "School item "
            flxItems.TextMatrix(i, PublisherNameCol) = "Litt.champ"
        End If
    Next
        ...
        With vp
            If printFormatOption <> 1 And printFormatOption <> 4 Then
                ...
                .StartTable
                    ...
                    '.TableCell(tcText, 2, 8) = "{\b " & Format(CN(0).recs!NetAmount, "##,##0.00") & " }"
            ...
            If PageNum = vp.PageCount Then
                With vp
                    .FontBold = True
                       ...
                       '     .TableCell(tcText, 1, 1) = "* ALL SAMPLE PAPER, C.B.S.E. & SBPD(MODEL PAPER 2012) WILL RETURN FROM 01-MAY TO 10-MAY-2012."
                        .EndTable
                    .FontBold = False
                End With
            End If
        ...
        Case Else:
            With vp
                ...
                .StartTable
                   ...
                   ' .TableCell(tcText, 1, 2) = "{\par\i prepared by \par\b " & Right(PREPARER(0), 10) & " \b0 \par checked by }"
                    .TableCell(tcText, 1, 3) = "{\b " & " \par\par\par\b0 Auth. Signatory }"
                   ' .TableCell(tcText, 1, 3) = "{\b\par For " & CompanyName & " \par\par\par\b0 Auth. Signatory }"
                .EndTable
            End With
    End Select
...
Private Sub cmdTransportForwarding_Click()
    ...
    With vp
        ...
        .StartTable
            ...
            .TableCell(tcText, 1, 3) = "{" & Salutation & " \b " & CN(0).recs!Name & "" & "" & " } \par " & CN(0).recs!Address & " \par " & CN(0).recs!City & ". Ph: " & CN(0).recs!Phones & " \par\par Through, " & CN(0).recs!TName & "\par " & CN(0).recs!Taddress & " \par " & CN(0).recs!TCity & " }"
        .EndTable
        .StartTable
            .TableBorder = tbBottom
            .TableCell(tcCols) = 1: .TableCell(tcRows) = 1
